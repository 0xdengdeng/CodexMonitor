...
BETA_DESCRIPTION="${BETA_DESCRIPTION:-启航AI智慧平台 iOS beta build for external testing.}"
...
REVIEW_NOTES="${REVIEW_NOTES:-启航AI智慧平台 iOS beta build for external testing.}"
SKIP_BUILD=0
SKIP_SUBMIT=0
...
Options:
  ...
  --ipa <path>               IPA path (default: src-tauri/gen/apple/build/arm64/启航AI智慧平台.ipa)
...
Examples:
  ...
  ./scripts/release_testflight_ios.sh --skip-build --ipa "src-tauri/gen/apple/build/arm64/启航AI智慧平台.ipa"
USAGE
}
...
if [[ -z "$IPA_PATH" ]]; then
  IPA_PATH="src-tauri/gen/apple/build/arm64/启航AI智慧平台.ipa"
fi
