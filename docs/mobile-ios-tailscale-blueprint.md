# AgentDesk iOS Remote Blueprint (Tailscale + TCP)

This document is the canonical runbook for iOS remote usage with a desktop-hosted AgentDesk backend over Tailscale.
...
1. Desktop AgentDesk hosts the daemon and executes Codex workflows.
2. iOS AgentDesk connects to the desktop daemon using `remoteBackendHost` + token.
...
- Desktop AgentDesk is installed and able to run local workspaces.
