...
describe("tauri invoke wrappers", () => {
  ...
  it("lists generated images with optional filters", async () => {
    const invokeMock = vi.mocked(invoke);
    invokeMock.mockResolvedValueOnce([]);

    await listGeneratedImages({ workspaceId: "ws-1", threadId: "thread-1" });

    expect(invokeMock).toHaveBeenCalledWith("list_generated_images", {
      workspaceId: "ws-1",
      threadId: "thread-1",
    });
  });
