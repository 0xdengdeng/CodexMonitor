    ...
    const menu = await Menu.new({
      items: [
        ...
        await MenuItem.new({
          text: t(
            nextScope === "workspace"
              ? "prompts.moveToWorkspace"
              : "prompts.moveToGeneral",
          ),
      ...
      <div className={`prompt-row${isHighlighted ? " is-highlight" : ""}`} key={key}>
        ...
        {pendingDeletePath === prompt.path && (
          <div className="prompt-delete-confirm">
            <span>{t("prompts.deleteConfirm")}</span>
            <button
              type="button"
              className="ghost prompt-action"
    ...
    <PanelShell
      ...
      headerRight={
        <PanelMeta className="prompt-panel-meta">
          {hasPrompts
            ? t(totalCount === 1 ? "prompts.countOne" : "prompts.countMany", {
                count: totalCount,
              })
            : t("prompts.none")}
        </PanelMeta>
      ...
      <div className="prompt-panel-scroll">
              ...
              <label className="prompt-editor-label">
                ...
                <SelectMenu
                  ...
                  onValueChange={(nextValue) =>
                    updateEditor({
                      scope: nextValue as PromptEditorState["scope"],
                    })
                  }
                  disabled={editor.mode === "edit"}
                  options={[
                    { value: "workspace", label: t("prompts.scopeWorkspace") },
                    { value: "global", label: t("prompts.scopeGeneral") },
                  ]}
                />
              </label>
            </div>
        ...
        <div className="prompt-section">
          <div className="prompt-section-header">
            <div className="prompt-section-title">{t("prompts.workspaceSection")}</div>
            <button
              type="button"
          ...
          {workspacePrompts.length > 0 ? (
            ...
            <div className="prompt-empty-card">
              ...
              <div className="prompt-empty-text">
                <div className="prompt-empty-title">{t("prompts.noWorkspace")}</div>
        ...
        <div className="prompt-section">
          <div className="prompt-section-header">
            <div className="prompt-section-title">{t("prompts.generalSection")}</div>
            <button
              type="button"
              className="ghost icon-button prompt-section-add"
          ...
          {globalPrompts.length > 0 ? (
            ...
            <div className="prompt-empty-card">
              <ScrollText className="prompt-empty-icon" aria-hidden />
              <div className="prompt-empty-text">
                <div className="prompt-empty-title">{t("prompts.noGeneral")}</div>
                <div className="prompt-empty-subtitle">
