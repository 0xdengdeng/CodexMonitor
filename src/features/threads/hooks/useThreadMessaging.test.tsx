...
describe("useThreadMessaging telemetry", () => {
  ...
  it("forwards the selected model and reasoning effort to turn/start", async () => {
    const { result } = renderHook(() =>
      useThreadMessaging({
        activeWorkspace: workspace,
        activeThreadId: "thread-1",
  ...
  it("localizes built-in fast mode replies", async () => {
    const dispatch = vi.fn();
    const onSelectServiceTier = vi.fn();
    const { result } = renderHook(() =>
      useThreadMessaging({
  ...
  it("localizes the interrupt assistant message", async () => {
    const dispatch = vi.fn();
    const { result } = renderHook(() =>
      useThreadMessaging({
        activeWorkspace: workspace,
