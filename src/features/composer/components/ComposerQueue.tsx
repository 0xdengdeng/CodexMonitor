              ...
              {item.text ||
                (item.images?.length
                  ? item.images.length === 1
                    ? t("composer.queue.image")
                    : t("composer.queue.images")
                  : "")}
              {item.images?.length
                ? ` · ${t(
                    item.images.length === 1
                      ? "composer.queue.imageCountOne"
                      : "composer.queue.imageCountMany",
  ...
  return (
    <div className="composer-queue-menu-wrap" ref={menu.containerRef}>
      <button
        ...
        aria-label={t("composer.queue.itemMenu")}
        aria-haspopup="menu"
      ...
      {menu.isOpen && (
        <PopoverSurface className="composer-queue-item-popover" role="menu">
          <PopoverMenuItem onClick={handleEdit}>{t("common.edit")}</PopoverMenuItem>
          <PopoverMenuItem onClick={handleDelete}>{t("common.delete")}</PopoverMenuItem>
        </PopoverSurface>
      )}
    </div>
  );
