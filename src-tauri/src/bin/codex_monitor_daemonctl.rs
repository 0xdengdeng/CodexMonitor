...
const EXPECTED_DAEMON_NAME: &str = "agentdesk-daemon";
...
const APP_IDENTIFIER: &str = "com.agentdesk.app";
...
fn resolve_token(token_arg: Option<&str>, settings: Option<&AppSettings>) -> Option<String> {
    trim_non_empty(token_arg)
        .or_else(|| trim_non_empty(env::var("AGENTDESK_DAEMON_TOKEN").ok().as_deref()))
...
LISTEN 0      4096   0.0.0.0:4732      0.0.0.0:*    users:(("agentdesk-daemo",pid=12345,fd=7))
"#;
...
tcp        0      0 0.0.0.0:4732            0.0.0.0:*               LISTEN      6789/agentdesk-daemo
"#;
